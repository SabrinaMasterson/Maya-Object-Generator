...
scaleF_r = .1,\
    ...
    for i in range( 0, 50 ):
       ...
       #create cube
    	cmds.polySphere( sx=1, sy=1, r=1, n='sphere{}'.format(i))
    	...
    	#apply variabels
    	cmds.rotate( xRot, yRot, zRot, 'sphere{}'.format(i))
    	
    	cmds.scale(scaleF, scaleF, scaleF, 'sphere{}'.format(i))
    	
    	cmds.move( x, y, z, 'sphere{}'.format(i))
    	
    	cmds.sets( name='SphereMaterial{}'.format(i), renderable=True, empty=True )
    	cmds.shadingNode( 'phong', name='SphereShader{}'.format(i), asShader=True )
    	cmds.setAttr( 'SphereShader{}'.format(i)+'.color', r, g, b, type='double3' )
    	cmds.surfaceShaderList( 'SphereShader{}'.format(i), add='SphereMaterial{}'.format(i))
    	cmds.sets( 'sphere{}'.format(i), forceElement='SphereMaterial{}'.format(i))
PolySpheres()