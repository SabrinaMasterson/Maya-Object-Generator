...
scaleF_r = .1,\
    ...
    for i in range( 0, 50 ):
       ...
       #create cube
    	cmds.polyCube( sh=1, h=1, n='cube{}'.format(i))
    	...
    	#apply variabels
    	cmds.rotate( xRot, yRot, zRot, 'cube{}'.format(i))
    	
    	cmds.scale(scaleF, scaleF, scaleF, 'cube{}'.format(i))
    	
    	cmds.move( x, y, z, 'cube{}'.format(i))
    	
    	cmds.sets( name='CubeMaterial{}'.format(i), renderable=True, empty=True )
    	cmds.shadingNode( 'phong', name='CubeShader{}'.format(i), asShader=True )
    	cmds.setAttr( 'CubeShader{}'.format(i)+'.color', r, g, b, type='double3' )
    	cmds.surfaceShaderList( 'CubeShader{}'.format(i), add='CubeMaterial{}'.format(i))
    	cmds.sets( 'cube{}'.format(i), forceElement='CubeMaterial{}'.format(i)) 
PolyCubes()      